...
public class ProfileMinAvgMaxBarPanel extends JPanel {

	private double[] minValues;
	private double[] maxValues;
	private double uncertainty;

	public ProfileMinAvgMaxBarPanel(double[] minValues, double[] maxValues,
			double uncertainty) {
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < minValues.length; i++) {
			...
			Color color0 = new Color(100, 100, baseColor.getBlue(), min);
			Color color1 = new Color(100, 100, baseColor.getBlue(), min
					+ (max - min) / 2);
			Color color2 = new Color(100, 100, baseColor.getBlue(), max);
			g2d.setColor(color0);
			g2d.fillRect(x, 0, elementWidthPx, (int) (height / 3));
			g2d.setColor(color1);
			g2d.fillRect(x, (int) (height / 3), elementWidthPx,
					(int) (2. * height / 3) - 1);
			g2d.setColor(color2);
			g2d.fillRect(x, (int) (2 * height / 3), elementWidthPx,
					(int) (height - 1));
