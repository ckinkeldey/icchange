...
public class ProfileAreaPanel extends JPanel {

	private double[] minValues;
	private double[] maxValues;
	private double uncertainty;

	public ProfileAreaPanel(double[] minValues, double[] maxValues,
			double uncertainty) {
	...
	protected void paintComponent(Graphics g) {
		...
		for (i = 0; i < minValues.length; i++) {
			int x = elementWidthPx / 2 + (i * elementWidthPx);
			int y = (int) ((int) height - 1 - (minValues[i] / 255. * height));
		...
		// Polygon minPolygon1 = new Polygon();
		// for (i = 1; i < minPolygon.npoints-1; i++) {
		// minPolygon1.addPoint(minPolygon.xpoints[i], minPolygon.ypoints[i]);
		// }
