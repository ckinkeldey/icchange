package net.g2lab.icchange.view.utils;
...
class MultiSliderUI extends MetalSliderUI {
	...
	protected void calculateThumbLocation() {
		...
		for (int i = 0; i < this.thumbCount; i++) {
			...
			if (slider.getOrientation() == JSlider.HORIZONTAL) {
				...
				this.thumbRects[i].x = valuePosition
						- (this.thumbRects[i].width / 2);
				this.thumbRects[i].y = trackRect.y;
			} else {
				int valuePosition = yPositionForValue(slider.getValueAt(i));
				this.thumbRects[i].x = trackRect.x;
				this.thumbRects[i].y = valuePosition
						- (this.thumbRects[i].height / 2);
			}
		}
	}
	...
	public void scrollByBlock(int direction) {
		synchronized (slider) {
			...
			((MultiSlider) slider).setValueAt(this.currentIndex, oldValue
					+ delta);
		}
	}
	...
	class MultiTrackListener extends BasicSliderUI.TrackListener {
		...
		public void mousePressed(MouseEvent e) {
			...
			for (int i = 0; i < MultiSliderUI.this.thumbCount; i++) {
				if (MultiSliderUI.this.thumbRects[i].contains(currentMouseX,
						currentMouseY)) {
					...
					case JSlider.VERTICAL:
						offset = currentMouseY
								- MultiSliderUI.this.thumbRects[i].y;
						break;
					case JSlider.HORIZONTAL:
						offset = currentMouseX
								- MultiSliderUI.this.thumbRects[i].x;
						break;
					}
					...
					if (bounded) {
						...
						neighbours[1] = ((i + 1) >= MultiSliderUI.this.thumbCount) ? -1
								: (i + 1);
						// findClosest(currentMouseX, currentMouseY, neighbours,
						// i);
					} else {
						MultiSliderUI.this.currentIndex = i;
			...
			MultiSliderUI.this.currentIndex = findClosest(currentMouseX,
					currentMouseY, neighbours, -1);
			thumbRect = MultiSliderUI.this.thumbRects[MultiSliderUI.this.currentIndex];
			MultiSliderUI.this.isDragging = false;
			((MultiSlider) slider).setValueIsAdjustingAt(
					MultiSliderUI.this.currentIndex, true);
		...
		private void setThumbBounds(int[] neighbours) {
			...
			case JSlider.VERTICAL:
				_trackTop = (neighbours[1] == -1) ? trackRect.y
						: MultiSliderUI.this.thumbRects[neighbours[1]].y
								+ halfThumbHeight;
				_trackBottom = (neighbours[0] == -1) ? trackRect.y
						+ (trackRect.height - 1)
						: MultiSliderUI.this.thumbRects[neighbours[0]].y
								+ halfThumbHeight;
				break;
			case JSlider.HORIZONTAL:
				_trackLeft = (neighbours[0] == -1) ? trackRect.x
						: MultiSliderUI.this.thumbRects[neighbours[0]].x
								+ halfThumbWidth;
				_trackRight = (neighbours[1] == -1) ? trackRect.x
						+ (trackRect.width - 1)
						: MultiSliderUI.this.thumbRects[neighbours[1]].x
								+ halfThumbWidth;
				break;
			}
		}
		...
		private int findClosest(int x, int y, int[] neighbours, int excluded) {
			...
			for (int i = 0; i < MultiSliderUI.this.thumbCount; i++) {
				...
				case JSlider.VERTICAL:
					...
					dyh = (MultiSliderUI.this.thumbRects[i].y + MultiSliderUI.this.thumbRects[i].height)
							- y;
					if (dyh <= 0) {
				...
				case JSlider.HORIZONTAL:
					...
					dxw = (MultiSliderUI.this.thumbRects[i].x + MultiSliderUI.this.thumbRects[i].width)
							- x;
					if (dxw <= 0) {
						if (dxw >= leftmin) {
							leftmin = dxw;
			...
			// System.out.println("neighbours = " + neighbours[0] + ", " +
			// neighbours[1]);
			int closest = (Math.abs(leftmin) <= Math.abs(rightmin)) ? neighbours[0]
					: neighbours[1];
		...
		public void mouseDragged(MouseEvent e) {
			((MultiSlider) MultiSliderUI.this.slider)
					.setValueBeforeStateChange(((MultiSlider) MultiSliderUI.this.slider)
							.getValueAt(MultiSliderUI.this.currentIndex));
			int thumbMiddle = 0;
			boolean bounded = ((MultiSlider) slider).isBounded();
			...
			case JSlider.VERTICAL:
				...
				if (bounded) {
					...
					neighbours[1] = ((idx + 1) >= MultiSliderUI.this.thumbCount) ? -1
							: (idx + 1);
				...
				((MultiSlider) slider).setValueAt(
						MultiSliderUI.this.currentIndex,
		...
		public void mouseReleased(MouseEvent e) {
			...
			if (slider.getSnapToTicks()) {
				MultiSliderUI.this.isDragging = false;
				((MultiSlider) slider).setValueIsAdjustingAt(
						MultiSliderUI.this.currentIndex, false);
			} else {
				((MultiSlider) slider).setValueIsAdjustingAt(
						MultiSliderUI.this.currentIndex, false);
				MultiSliderUI.this.isDragging = false;
