 ...
 * 
 */
public class ThicknessLines implements UncertaintyLines {
	...
	public ThicknessLines(Collection<UncertaintyLineStringFeature> boundaries, Envelope envelope) {
...
//		LineString outerRing = ((com.vividsolutions.jts.geom.Polygon) feature.getGeometry()).getExteriorRing();
		Polygon polygon = createPolygon(boundary, scaleX, scaleY);
		
		g2d.setStroke(new BasicStroke((float) (3*Math.random())));
		g2d.setColor(DASH_COLOR);
		g2d.draw(polygon);
	}

//	private BasicStroke createDashedStroke(Graphics2D g2d) {
//		double uncertainty = Math.random();
//		float dashSizePx = (float) (14 * uncertainty);
//		float[] dash = new float[] {};
//		if (dashSizePx > 0) {
//			dash = new float[] {1, dashSizePx};
//		} 
//		return new BasicStroke(STROKE_WIDTH, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_ROUND, 10, dash, 0);
//	}
//	
//	private void drawHoles(Graphics2D g2d, UncertaintyLineStringFeature feature, double scaleX,
//			double scaleY) {
//		Polygon polygon;
//		int numHoles = ((com.vividsolutions.jts.geom.Polygon) feature.getGeometry()).getNumInteriorRing();
//		for (int i = 0; i < numHoles; i++) {
//			LineString interiorRing = ((com.vividsolutions.jts.geom.Polygon) feature.getGeometry()).getInteriorRingN(i);
