...
import net.g2lab.icchange.view.profile.TestFrameHorizontal;
 ...
 * 
 */
public class HistogramVerticalPanelTest {
	...
	public HistogramVerticalPanelTest() {
		try {
			int numDates = 2;
			int numObjects = 0;
			...
			for (int date = 0; date < components.length; date++) {
				for (int obj = 0; obj < components[0].length; obj++) {
					...
					JComponent component = new HistogramVerticalPanel(
							new double[][] { histograms.get(1)[obj],
									histograms.get(2)[obj],
									histograms.get(3)[obj],
									histograms.get(4)[obj],
			...
			new TestFrameHorizontal(components);
		} catch (Exception e) {
			LOG.error(e, e);
		}
	}
